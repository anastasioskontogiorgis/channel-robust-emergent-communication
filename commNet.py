...
from utils import generate_noise, emulate_comm_quality, drop_messages, jumble_messages, comm_delay_n_hops
...
class CommNetMLP(nn.Module):
    ...
    def __init__(self, args, num_inputs):
        ...
        self.args = args
        #TODO: sanity check
        if self.args.ic3net  and self.args.hard_attn:
            self.name = 'ic3net'
        elif self.args.commnet:
            self.name = 'commnet'        
    ...
    def forward(self, x, info={}):
        ...
        # Initialize buffer for message delay
        hop_delay_buffer = deque(maxlen=self.args.delay_step)  # maxlen = 2 - two communication rounds
        ...
        # Hard Attention - action whether an agent communicates or not (at the the next env step)
        ...
        for i in range(self.comm_passes):
            ...
            if self.args.comm_constraints == 'simple' and self.args.noise_level > 0.0: 
                ...
                c = emulate_comm_quality(c, 
                                     self.args.noise_type, 
                                     self.args.noise_mean)
            elif self.args.comm_constraints == 'drops':
                c = drop_messages(c, self.args.drop_prob_whole, 
                                  self.args.drop_prob_part)
            elif self.args.comm_constraints == 'jumble':
                c = jumble_messages(c, self.args.jumble_prob)
            if self.args.comm_constraints == 'delay':
                c = comm_delay_n_hops(c, hop_delay_buffer,
